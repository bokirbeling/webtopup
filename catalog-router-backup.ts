...
function flatProductResponse(pricedProduct: PricedProduct) {
  const provider = pricedProduct.product.provider.toLowerCase().replace(/[^a-z0-9]/g, "-");
  const localImagePath = `/product-images/${provider}.png`;
  const fallbackImageUrl = pricedProduct.product.metadata?.image_url || "";
  
  return {
    id: pricedProduct.product.id,
    sku_digiflazz: pricedProduct.product.skuDigiflazz,
    name: pricedProduct.product.name,
    category: pricedProduct.product.category,
    provider: pricedProduct.product.provider,
    base_price_minor: pricedProduct.basePriceMinor,
    final_price_minor: pricedProduct.finalPriceMinor,
    is_active: pricedProduct.product.isActive,
    image_url: localImagePath,
    fallback_image_url: fallbackImageUrl,
    metadata: pricedProduct.product.metadata,
    main_category: pricedProduct.product.category,
    sub_category: pricedProduct.product.category,
    product_type: pricedProduct.product.category
  };
}
...
export function createCatalogRouter(dependencies: CatalogRouterDependencies, productCache: ProductCacheService) {
  ...
  catalogRouter.get("/products", async (request, response, next) => {
    try {
      ...
      const result = await productCache.getPaginatedProducts(roleType, page, limit, search, category);
