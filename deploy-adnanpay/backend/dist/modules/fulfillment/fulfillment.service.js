...
exports.FulfillmentValidationError = void 0;
exports.createFulfillmentService = createFulfillmentService;
const node_crypto_1 = require("node:crypto");
const order_service_1 = require("../order/order.service");
class FulfillmentValidationError extends Error {
    constructor(message) {
        super(message);
...
function isLiveDigiflazzConfigured(config) {
...
function signDigiflazz(username, apiKey, refId) {
    return (0, node_crypto_1.createHash)("md5").update(username + apiKey + refId).digest("hex");
}
function mapDigiflazzStatus(rawStatus) {
...
function extractCallbackData(payload) {
    return toRecord(payload.data ?? payload);
}
function extractEventKey(data) {
...
function digiflazzPayloadFor(order, username, apiKey, refId) {
    return {
        username,
...
function modeResponsePayload(mode, payload) {
    return {
        ...payload,
        provider_mode: mode
    };
...
function createFulfillmentService(options) {
    ...
    return {
        async triggerPaidOrderFulfillment(input) {
            const now = clock();
            ...
            if (existing) {
                return {
                    ...
                    providerReference,
                    providerMode: existing.responsePayload.provider_mode ?? providerMode
                };
            }
        ...
        async handleDigiflazzCallback(payload) {
            const now = clock();
            ...
            if (registration.duplicate) {
                return { code: "DUPLICATE", message: "Duplicate Digiflazz callback ignored." };
            }
            if (!fulfillment) {
            ...
            if (status === "success" || status === "failed") {
                try {
                    ...
                    if (error instanceof order_service_1.OrderTransitionError) {
                        ...
                        return { code: "IGNORED", message: "Callback did not mutate order due to monotonic transition guard." };
                    }
                    throw error;
                }
            }
            ...
            return { code: "PROCESSED", message: "Digiflazz callback processed." };
        }
    };
}
